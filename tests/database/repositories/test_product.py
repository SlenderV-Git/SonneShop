...
class TestProductRepository:
    @pytest.mark.asyncio
    async def test_select_products(
        self, gateway: DBGateway, products: Sequence[ProductModel]
    ):
    ...
    async def test_delete_products(
        ...
        assert (
            all([await gateway.product().delete(product.id) for product in products])
            is True
        )
